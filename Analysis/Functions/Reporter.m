function [reporteravg] = Reporter(tracks)
...
%    plot(movmean(TauValues,5),(movmean(ReporterMeans,5)-min(ReporterMeans))/(max(ReporterMeans)-min(ReporterMeans)))
    ...
    if ReporterMax >= 0
        ...
        Timeshift = TauValues(maxreporterindex);
        reporterIndicator = 1;
    end
    ...
    results = [dummyreporterindicator dummytimeshift dummytimebin];
...
plot(reporteravg(:,1).*timescale,reporteravg(:,2))
...
%  plot(movmean(X4_2_r(:,1)*3,4),movmean(X4_2_r(:,2)/(min(X4_2_r(:,2))),4))
% hold on
% plot(movmean(X5_5_r(:,1)*3,4),movmean(X5_5_r(:,2)/(min(X5_5_r(:,2))),4))
% plot(movmean(X3_10_r(:,1)*3,4),movmean(X3_10_r(:,2)/(min(X3_10_r(:,2))),4))
% plot(movmean(X6_15_r(:,1)*3,4),movmean(X6_15_r(:,2)/(min(X6_15_r(:,2))),4))
% plot(movmean(X6_30_r(:,1)*3,4),movmean(X6_30_r(:,2)/(min(X6_30_r(:,2))),4))
